...
newnamedic ={
    ...
    "PPR+OSR-Graph":"SBE-PPR+OSAR-Graph",
...
def generation(PATHNUM,dataset):
    ...
    for llm in llmlist:
        ...
        for model in modelist:
            for prtype,result in resultlist.items():
                ...
                try:
                    # print("使用新数据：")
                    # if llm == "qwen2-7b":
                    # if aftername=="SBE-PPR+OSAR-Graph":
        ...
        # 加一个Instance SBE-PPR+OSAR-Graph
        aftername="GraphRAG-M"
        jsonpath = f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/metaQA/PPR/GraphRAG/{llm}.json"
        f1, hit32 = processSingleJson(jsonpath)
        print("使用新数据成功！")
        # F1 += f1
        ...
        df.to_excel(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result/{dataset}-16Instance-{llm}-@{PATHNUM}.xlsx",index=False)
...
def GenerationImage(rootpath, saveImgpath,modeNUM):
    custom_colors = [
        ...
        "#42CFFE", "#0487e2",
        "#C8EAD1","#73C088","#397D54", "#3D4F2F","#B7B7EB"
    ]
    # 读取Excel数据
    ...
    for i in range(0,3):
        ...
        #     dataset = "WebQuestion"
        llmpath = f"{rootpath}/{dataset}-16Instance-{llmlist[i]}-@{PATHNUM}.xlsx"
        ...
        ax[i].axvspan(x[10] + 0.8, x[14]+1.5, facecolor='#E3E9D7', alpha=0.3)
        ...
        ax[i].set_xticklabels(range(1,17))  # 隐藏 X 轴标签
        xticks = range(1, 17)  # 从 1 到 16 的刻度  
        # ax[i].set_xticks(xticks)  

        # 设置 x 轴刻度标签  
        # 使用列表推导式来格式化标签  
        xticklabels = [f"GRAG-M" if i == 16 else str(i) for i in xticks]  
        ax[i].set_xticklabels(xticklabels) 
        # 将前15个标签旋转  
        for label in ax[i].get_xticklabels():  # 选择前15个标签  
            label.set_rotation(60)                # 旋转  
        label.set_rotation(0) 
        ax[i].tick_params(axis='x', which='major', labelsize=30, width=3,length=20)
        ax[i].tick_params(axis='y', which='major', labelsize=40, width=3,length=20)
        ax[i].xaxis.labelpad = 10
        if dataset == "CWQ":
            ax[i].text(-0.5, 0.25, "No.", ha='right', va='center', fontsize=30, fontweight='bold')
        else:
            ax[i].text(-0.5, 0.23, "No.", ha='right', va='center', fontsize=30, fontweight='bold')
    ...
    plt.subplots_adjust(top=0.95, left=0.08, right=0.98,bottom=0.35)
