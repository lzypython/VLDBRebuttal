...
def generation(PATHNUM,dataset):
    ...
    for llm in llmlist:
        ...
        df.to_excel(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result/{dataset}-Graph-{llm}-@{PATHNUM}.xlsx",index=False)
...
def GenerationImage(rootpath, saveImgpath,modeNUM):
    ...
    for i in range(0,4):
        ...
        #     dataset = "WebQuestion"
        llmpath = f"{rootpath}/{dataset}-Graph-{llmlist[i]}-@{PATHNUM}.xlsx"
...
if __name__ == "__main__":
    ...
    llmlist = ["qwen2-7b","glm4-9b","qwen2-70b","llama3.3-70b"]
    ...
    # for llm in llmlist:
    # for dataset in datasetlist:
    GenerationImage(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result",f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result/metaQA-Graph-Generation-Hit@{PATHNUM}.pdf",len(llmlist))
    # print(f"{llm}画图完成！")