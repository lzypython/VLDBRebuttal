    ...
    with open(jsonpath, "r") as f:
        ...
        for i in range(len(infos["eval_info"])):
            ...
            if dataset=="WebQuestion":
                ...
                hop = id2hop[id]
            if hoptype == "OneHop" and hop == 1:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
            elif hoptype == "TwoHop" and hop == 2:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
            elif hoptype == "MultiHop" and hop > 2:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
    F1 = F1/count
...
def generation(PATHNUM,hoptype):
    for llm in llmlist:
        ...
        df.to_excel(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result/Average-{llm}-@{PATHNUM}-{hoptype}.xlsx",index=False)
...
def GenerationImage(rootpath, saveImgpath):
    ...
    for k in range(0,1):
        for i in range(0,3):
            if i==0:
                hoptype = "OneHop"
                xlabelname = "#Hop=1 (46.69%)"
            elif i ==1:
                hoptype = "TwoHop"
                xlabelname = "#Hop=2 (28.85%)"
            else:
                hoptype = "MultiHop"
                xlabelname = "#Hop>2 (24.46%)"
            llmpath = f"{rootpath}/Average-{llmlist[i]}-@{PATHNUM}-{hoptype}.xlsx"
...
if __name__ == "__main__":
    ...
    llmlist = ["qwen2-7b","qwen2-7b","qwen2-7b"]
    datasetlist = ["CWQ","webqsp","GrailQA","WebQuestion"]
    resultlist = {
        "GSR":"SPR",
        "OSR-EEMS":"SPR/EMB",
    ...
    # for llm in llmlist:
    # for dataset in datasetlist:
    GenerationImage(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result",f"Average-Hop-Hit{PATHNUM}.pdf")
