...
def processSingleJson(jsonpath,dataset,hoptype):
    ...
    with open(jsonpath, "r") as f:
        ...
        for i in range(len(infos["eval_info"])):
            ...
            if hoptype == "OneAnswer" and answernum == 1:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
            elif hoptype == "TwoAnswer" and answernum==2:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
            elif hoptype == "MoreThan-TwoAnswer" and answernum>2:
                F1 += data["F1"]
                Hit += data["HR"]
                count += 1
    F1 = F1/count
...
def GenerationImage(rootpath, saveImgpath):
    ...
    for k in range(0,1):
        ...
        # generation(PATHNUM,"MoreThan-TenAnswer")
            if i==0:
                hoptype = "OneAnswer"
                xlabelname = "#Ans=1 (65.69%)"
            elif i ==1:
                hoptype = "TwoAnswer"
                xlabelname = "#Ans=2 (9.48%)"
            elif i ==2:
                hoptype = "MoreThan-TwoAnswer"
                xlabelname = "#Ans>2 (24.83%)"
            llmpath = f"Average-{llmlist[i]}-@{PATHNUM}-{hoptype}.xlsx"
            df = pd.read_excel(llmpath)
            Hitlist = df[f"F1"].tolist()
            ...
            ax[i].set_ylabel(f"F1", fontsize=40, fontweight='bold')
            # 隐藏 X 轴刻度和标签
            ...
            ax[i].text(-0.5, 0.1, "No.", ha='right', va='center', fontsize=35, fontweight='bold') 
...
if __name__ == "__main__":
    ...
    for dataset in datasetlist:
        ...
        if dataset=="WebQuestion":
            ...
            with open(datasetpath, "r") as f:
                ...
                for data in hopinfo:
                    question = normalize(data["question"])
                    questin2Answer[question] = len(data["answers"])
        else:
            # 根据id找hop
            with open(datasetpath, "r") as f:
                hopinfo = json.load(f)
                for data in hopinfo:
                    id2Answer[str(data["id"])] = len(data["answers"])
    ...
    #     generation(PATHNUM,"OneAnswer")
    #     generation(PATHNUM,"TwoAnswer")
    #     generation(PATHNUM,"MoreThan-TwoAnswer")
    # for llm in llmlist:
    # for dataset in datasetlist:
    GenerationImage(f"/back-up/gzy/dataset/VLDB/Instance/NewGeneration/Result",f"Average-Answer-F1{PATHNUM}.pdf")
